...
# 讀取 json
json <- fromJSON(file = "data/dataset/version_1.json")
...
for(i in 1:nrow(df)){
  ...
  # 取得出現頻率最高的278個常用字
  top_words <- head(sort(word_freq, decreasing = TRUE), 278)
  
  # 移除數字
  segmented_text <- names(top_words)
  
  # 移除空白
  segmented_text <- segmented_text[-1]

  # 將斷詞透過空格合併，以符合訓練資料格式
  segmented_text <- stringr::str_c(segmented_text, collapse = " ") %>% c()
...
# 建立 csv 資料夾
dir.create('data/csvfortrain', recursive = TRUE, showWarnings = FALSE)

# 儲存 csv
write.csv(df_csv, 'data/csvfortrain/df_jieba768_fre278.csv', row.names = FALSE, quote = FALSE)
