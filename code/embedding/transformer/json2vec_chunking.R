...
for(i in 1:nrow(df)){
    ...
    # 計數
    quotient <- len_text %/% 510  # 計算商數
    remainder <- len_text %% 510  # 計算餘數
    
    # 餘數無條件進位
    if (remainder > 0) {
        quotient <- quotient + 1  # 餘數無條件進位
    }
    ...
    for (j in 1:quotient){
        ...
        input_ids <- torch$tensor(tokenizer$encode(text[(j - 1) * 510 + 1 : j * 510], truncation=TRUE))$unsqueeze(as.integer(0))
    
        # 輸出 hidden_state
        outputs <- model(input_ids)
    
        # 建立 embeddings
        all_embeddings[[j]] <- outputs[["last_hidden_state"]][0][0]$detach()$numpy()
    }
...
# 儲存 csv
write.csv(df_csv, 'csv/df_bert_chunking.csv', row.names = FALSE, quote = FALSE)
